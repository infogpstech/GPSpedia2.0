...
async def handle_route(route):
    ...
    if CATALOG_SERVICE_URL in url and action == "getCatalogData":
        # Introduce a small delay to simulate network latency and test the loading spinner
...
async def main():
    async with async_playwright() as p:
        # --- Part 1: Verify Startup Race Condition Fix ---
        print("--- Verifying Startup Race Condition Fix ---")
        ...
        try:
            ...
            print("Waiting for spinner to be visible...")
            await expect(page.locator(".spinner")).to_be_visible(timeout=5000)
            print("✅ SUCCESS: Loading spinner is visible.")

            print("Waiting for content cards to be rendered...")
            await expect(page.locator(".card").first).to_be_visible(timeout=10000)
            print("✅ SUCCESS: Content cards are visible. Startup race condition is fixed.")

        except Exception as e:
            print(f"❌ STARTUP TEST FAILED: {e}")
            await page.screenshot(path="solution_verification_error.png")
            raise
        finally:
            await page.close()
            await context.close()

        # --- Part 2: Verify Mobile Layout Fix ---
        print("\n--- Verifying Mobile Layout Fix ---")
        ...
        try:
            print("Navigating to index.html on mobile...")
            await page.goto("http://localhost:8000/index.html", wait_until="domcontentloaded")

            await expect(page.locator(".card").first).to_be_visible(timeout=10000)
            print("✅ Application loaded and content is visible on mobile.")
            ...
            screenshot_path = 'mobile_layout_verification.png'
            await page.screenshot(path=screenshot_path, full_page=True)
            print(f"📸 Screenshot saved to {screenshot_path}")
            print("📝 Please inspect screenshot to confirm mobile layout fix.")
